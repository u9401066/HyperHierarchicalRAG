...
class QueryProcessor:
    ...
    async def query_hybrid(
        ...
        if use_hypergraph and lightrag_candidates:
            hypergraph_expanded = await self._expand_via_hyperedges(
                lightrag_candidates, max_hops=2
            )
    ...
    async def _extract_global_keywords(self, query: str) -> List[str]:
        ...
        theme_words = [
            "compare", "contrast", "overview", "summary", "trend",
            "analysis", "relationship", "impact", "effect", "cause",
            "mechanism", "process", "system", "approach", "method",
        ]
    ...
    async def _expand_via_hyperedges(
        ...
        if self._repository:
            ...
            for node in seed_nodes:
                connected = await self._repository.find_connected_nodes(
                    node.id, max_hops=max_hops
                )
    ...
    async def _store_evolved_memory(self, evolve_result) -> None:
