...
from hyperhierarchical_rag.visualization.query_path_viz import QueryStep, QueryTrace
...
class RAGEngine:
    ...
    async def initialize(self) -> Dict[str, Any]:
        ...
        if self.config.enable_visualization:
            self._graph_viz = HypergraphVisualizer(output_dir=self.config.storage.viz_dir)
            self._path_viz = QueryPathVisualizer(output_dir=self.config.storage.viz_dir)
    ...
    async def _init_adapters(self) -> None:
        ...
        self._kg_adapter = LightRAGKGAdapter(self._lightrag.chunk_entity_relation_graph)

        # Vector Store Adapters - wrap LightRAG's vector databases
        self._entities_vdb = VectorStoreAdapter(self._lightrag.entities_vdb, namespace="entities")
        self._relationships_vdb = VectorStoreAdapter(
            self._lightrag.relationships_vdb, namespace="relationships"
        )
        self._chunks_vdb = VectorStoreAdapter(self._lightrag.chunks_vdb, namespace="chunks")

        # Text Chunks Adapter - combines KV storage + vector search
        self._text_chunks_adapter = TextChunksAdapter(
            kv_storage=self._lightrag.text_chunks, vector_storage=self._lightrag.chunks_vdb
        )
    ...
    async def _init_hgmem_services(self) -> None:
        ...
        # Initialize persistence repository for memory points (Unified SQLite)
        ...
        # 實際上這現在是備案或主要本地存儲
        self._hypergraph_repo = SQLiteUnifiedRepository(str(hypergraph_db_path))
        ...
        if self._kg_adapter and self._text_chunks_adapter:
            self._memory_retriever = MemoryPointwiseRetriever(
                kg_adapter=self._kg_adapter, text_chunks_adapter=self._text_chunks_adapter
            )
    ...
    async def _init_llm(self) -> Callable:
        ...
        if llm_config.provider == "openai":
            ...
            async def llm_func(prompt: str, system_prompt: str | None = None, **kwargs: Any) -> str:
                # OpenAI uses system_prompt if provided
                if system_prompt:
                    kwargs["system_prompt"] = system_prompt

                return str(
                    await openai_complete_if_cache(model=llm_config.model, prompt=prompt, **kwargs)
                )
    ...
    async def _init_lightrag(self) -> Any:
        ...
        if absent_entities and self._sync_service:
            try:
                collected, _ = await self._sync_service.collect_absent_entities_relationships(
                    absent_entities_hyperedges_kv=absent_entities, context_info=retrieved_context
                )
        ...
        if self._memory_evolver and self._hypergraph_repo:
            try:
                ...
                if expanded_context:
                    ...
                    logger.info(
                        f"Hypergraph expansion: {expanded_result.get('new_entities', 0)} new entities discovered"
                    )
            except Exception as e:
        ...
        if self._memory_retriever and self._memory_evolver:
            try:
                ...
                if memory_points_list:
                    retrieval_result = (
                        await self._memory_retriever.get_memory_pointwise_related_info(
                    ...
                    trace.add_step(
                        QueryStep(
                            ...
                            description="Retrieved memory-related chunks",
                            keywords=[],
                        )
                    )
            except Exception as e:
        ...
        if self._lightrag:
            try:
                ...
                if hasattr(graph_storage, "_graph"):
                    ...
                    logger.info(
                        f"Extracted {len(nodes)} nodes, {len(edges)} edges from LightRAG KG"
                    )
            except Exception as e:
    ...
    async def _expand_via_hypergraph(
        ...
        if discovered_list:
            ...
            context_lines.append(
                f"Discovered {len(discovered_list)} additional entities via memory hyperedges:"
            )
            context_lines.append("")
    ...
    async def get_knowledge_graph(
        ...
        try:
            res = await self._lightrag.get_knowledge_graph(
