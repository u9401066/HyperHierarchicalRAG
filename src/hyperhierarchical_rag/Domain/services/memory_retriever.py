...
"""
    ...
    async def _collect_related_entities(
        ...
        truncate_attribute_by_token_size(
            related_entities_data, "description", query_param.max_token_for_entity_description
        )
    ...
    async def _retrieve_chunks_per_memory_point(
        ...
        for mp in memory_points:
            ...
            selected_inner = await self._select_chunks_by_similarity(
                inner_chunk_ids, mp_info or query, query_param.max_inner_chunks_per_memory_point
            )
            ...
            selected_outer = await self._select_chunks_by_similarity(
                outer_chunk_ids, mp_info or query, query_param.max_outer_chunks_per_memory_point
            )
            final_chunk_ids.update(selected_outer)

            pointwise_chunks.append(
                {"inner_chunks": list(selected_inner), "outer_chunks": list(selected_outer)}
            )
    ...
    async def _get_inner_chunks(
        ...
        for entity_name in mp:
            ...
            source_ids = set(
                split_string_by_multi_markers(str(node_data["source_id"]), [self.graph_field_sep])
            )
    ...
    async def _fetch_and_format_chunks(
        ...
        truncated = truncate_list_by_token_size(
            valid_chunks, key=lambda x: x["data"]["content"], max_token_size=max_token_size
        )
...
async def get_memory_pointwise_related_info(
    ...
    text_chunks_adapter = TextChunksAdapter(
        kv_storage=text_chunks_db, vector_storage=text_chunks_vdb
    )

    retriever = MemoryPointwiseRetriever(
        kg_adapter=knowledge_graph_inst, text_chunks_adapter=text_chunks_adapter
    )

    # 轉換查詢參數
    mp_param = MemoryQueryParam(
        max_inner_chunks_per_memory_point=getattr(
            query_param, "max_inner_chunks_per_memory_point", 3
        ),
        max_outer_chunks_per_memory_point=getattr(
            query_param, "max_outer_chunks_per_memory_point", 2
        ),
        max_text_chunks=getattr(query_param, "max_text_chunks", 20),
        max_token_for_final_text_chunks=getattr(
            query_param, "max_token_for_final_text_chunks", 4000
        ),
        max_token_for_entity_description=getattr(
            query_param, "max_token_for_entity_description", 200
        ),
    )
