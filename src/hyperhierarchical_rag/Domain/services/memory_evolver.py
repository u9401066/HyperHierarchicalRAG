    ...
    def _parse_memory_points_section(
        ...
        for match in matches:
            ...
            if len(parts) >= 2:
                ...
                objects = [
                    obj.strip().upper()
                    for obj in objects_str.split(object_delimiter)
                    if obj.strip()
                ]
        ...
        for idx_str, content in matches:
            try:
                ...
                if len(parts) >= 2:
                    ...
                    if objects and description:
                        updated.append(
                            (idx, MemoryPoint(involved_objects=objects, description=description))
                        )
            except ValueError:
                continue
...
class EnhancedMemoryEvolver(MemoryEvolver):
    ...
    def __init__(
        self,
        llm_func: Callable | None = None,
        knowledge_graph_adapter: Any | None = None,  # LightRAG adapter
        persistence_repo: Any | None = None,  # SQLiteHypergraphRepository
        **kwargs: Any,
    ) -> None:
        """
    ...
    def apply_reorganization(self, reorg_result: ReorganizeResult) -> None:
        ...
        self._memory_points = [
            p for i, p in enumerate(self._memory_points) if i not in indices_to_remove
        ]
        self._memory_points.extend(new_points)

        logger.info(
            f"Applied reorganization: removed {len(indices_to_remove)}, added {len(new_points)}"
        )
    ...
    async def _get_neighbors_from_kg(self, entity_name: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if hasattr(self.kg_adapter, "get_neighbors"):
                res = await self.kg_adapter.get_neighbors(entity_name)
    ...
    async def get_memory_point_info(self, mp_identifier: tuple[str, ...] | list[str]) -> str:
        ...
        # 在內部記憶點列表中查找
        for point in self._memory_points:
            if tuple(point.involved_objects) == mp_key or set(point.involved_objects) == set(
                mp_key
            ):
