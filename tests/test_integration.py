...
class TestInMemoryKGAdapter:
    ...
    async def test_upsert_and_get_node(self, adapter):
        ...
        await adapter.upsert_node("ENTITY_A", {
            "entity_type": "PERSON",
...
class TestInMemoryVectorStore:
    ...
    async def test_filter_lambda(self, store):
        ...
        results = await store.query(
            "AI",
            filter_lambda=lambda d: d.get("type") == "paper"
        )
...
"""
    ...
    async def test_collect_absent_entities(self, sync_service):
        ...
        collected_entities, collected_rels = await sync_service.collect_absent_entities_relationships(
...
class TestSQLiteHypergraphRepository:
    """Test SQLiteHypergraphRepository functionality."""
    ...
    def repo(self):
        ...
        os.close(fd)
        repo = SQLiteHypergraphRepository(db_path=path)
        yield repo
        # Cleanup
        try:
            os.unlink(path)
    ...
    async def test_find_by_keywords(self, repo):
        """Test keyword search."""
        await repo.upsert_node(HyperNode(
            id="node-ml",
            name="ML Node",
            level=NodeLevel.LOCAL,
    ...
    async def test_stats(self, repo):
        ...
        assert stats["nodes"]["local"] == 1
        assert stats["nodes"]["global"] == 1
