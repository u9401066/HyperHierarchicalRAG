...
async def main():
    ...
    print(
        f"   Nodes: {stats['nodes']['total']} (LOCAL: {stats['nodes']['local']}, GLOBAL: {stats['nodes']['global']})"
    )
    print(
        f"   Edges: {stats['edges']['total']} (binary: {stats['edges']['binary']}, n-ary: {stats['edges']['n_ary']})"
    )
    ...
    for query, mode in queries:
        ...
        print(
            f"   - Keywords: LOCAL={hr.get('local_keywords', [])}, GLOBAL={hr.get('global_keywords', [])}"
        )
        print(
            f"   - Candidates: {hr.get('total_candidates', 0)} (expanded +{hr.get('hypergraph_expanded', 0)})"
        )
    ...
    try:
        ...
        print("   Open in browser to view interactive graph!")
    except Exception as e:
